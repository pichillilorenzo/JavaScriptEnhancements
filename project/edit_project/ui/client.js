  ...
  if (fs.existsSync(settings_file)) {
    ...
    if(types.must_delete){
      for(let i = 0, length1 = types.must_delete.length; i < length1; i++){
        ...
        try {
          project_type_default_config = require('../../settings/'+types.must_delete[i]+'/default_config.js')
        } catch(e) {
          continue
        }
    ...
    if(types.must_add){
      for(let i = 0, length1 = types.must_add.length; i < length1; i++){
        let project_type_default_config =  {}
        try {
          project_type_default_config = require('../../settings/'+types.must_add[i]+'/default_config.js')
        } catch(e) {
          continue
        }
        if(project_type_default_config.flow_settings) {
